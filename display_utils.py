...
default_colors = np.clip((np.array([cmap(i) for i in np.linspace(0, 1, 30)]) * 255)[:, ::-1], 0, 255).astype(np.uint8)[:, 0:3]
...
def drawPoints(img, points, mask=None, text_scale=0, color_table=default_colors, point_ratio=2):
    shape = points.shape
    ...
    if points.shape[-1] == 3:
        ...
        valids = np.ones_like(points[:, 0])

    if not mask is None:
        valids = mask.astype(np.int32) * valids

    if img.max() > 1:
        img = img / 255.0

    if color_table is not None:
        color_table = np.array(color_table).astype(np.float)
        if color_table.max() > 1:
    ...
    for num in range(shape[0]):
        ...
        if color_table is not None:
            point_color = color_table[num]

        if (points[num] != np.array([0, 0])).any():
            cur_x = int(points[num][0])
            cur_y = int(points[num][1])

            img = cv2.circle(img, (cur_x, cur_y), point_ratio, color=point_color, thickness=cv2.FILLED, lineType=cv2.LINE_AA)

            if text_scale > 0:
                cv2.putText(img, "%d" % num, (cur_x, cur_y), cv2.FONT_HERSHEY_SIMPLEX, text_scale, (1.0, 1.0, 1.0), 1)
    return img

def drawLines(img, points, indices, mask=None, show_type="all", linewidth=2, color_table=default_colors):
    if isinstance(show_type, str):
        assert show_type == "all", "Only support show_type='all' of show_type=list"
        display_arr = np.ones(indices.shape[0]).astype(np.bool)
    elif isinstance(show_type, list) or isinstance(show_type, np.ndarray):
        display_arr = np.array(show_type).astype(np.bool)
    ...
    if not mask is None:
        valids = valids * mask.astype(np.int32)

    for i in range(indices.shape[0]):
        if valids[indices[i][0]] == 0 or valids[indices[i][1]] == 0 or not display_arr[i]:
            continue
        ...
        if not point_a.any() or not point_b.any():
            ...
            img = cv2.line(img, tuple(point_a), tuple(point_b), tuple(color_table[i]), linewidth, lineType=cv2.LINE_AA)
    return img

def drawRel(img, joints, rels, skeleton):
    return skeleton.drawRel(img, joints, rels)
